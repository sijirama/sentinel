    ...
    return (
        <div className="bg-[#080B0F] text-white min-h-screen p-4 font-poppins">
            ...
            <section className='mx-auto max-w-4xl '>
                ...
                <div className={`p-4 mb-6 rounded-sm text-start font-bold ${areAllOperational ? 'bg-green-500' : 'bg-red-500'
                    }`}>
                    {areAllOperational
                ...
                {Object.entries(statusData.siteStatuses).map(([key, siteStatus]) => (
                    <div key={key} className="mb-8">
                        <Card className="mb-4">
                            <div className='mb-4'>
                                <p className='font-bold text-xl capitalize '>{siteStatus.site.name}</p>
                            </div>
                            <p className="text-tremor-default flex items-center justify-between mb-2">
                                <span className="text-tremor-content-strong dark:text-dark-tremor-content-strong font-medium"> <a target='_blank' href={siteStatus.site.url} >{siteStatus.site.url}</a> </span>
                                <span className="text-tremor-content dark:text-dark-tremor-content">uptime {siteStatus.uptime.toFixed(2)}%</span>
                            </p>
                            <Tracker data={mapStatusToTrackerData(siteStatus.statuses)} className="mt-2 mb-4" />
                            <AreaChart
                                className="h-80"
                                data={mapStatusToChartData(siteStatus.statuses)}
                                index="date"
